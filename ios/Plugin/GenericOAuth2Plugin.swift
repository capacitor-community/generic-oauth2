...
public class GenericOAuth2Plugin: CAPPlugin {
  ...
  override public func load() {
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.handleRedirect(notification:)), name: .capacitorOpenURL,
      object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.handleRedirect(notification:)),
  ...
  @objc func refreshToken(_ call: CAPPluginCall) {
    ...
    oauthSwift.renewAccessToken(withRefreshToken: refreshToken, parameters: parameters) { result in
      ...
      case .success(let tokenSuccess):
        do {
          let jsonObj =
            try JSONSerialization.jsonObject(with: tokenSuccess.response!.data, options: [])
            as! JSObject
      ...
      case .failure(let error):
        ...
        case .requestError(let underlyingError, _):
          ...
          do {
            ...
            {
              call.reject(
                json["error"] as? String ?? self.ERR_GENERAL, String(errorCode), underlyingError,
                json)
            }
          } catch {
  ...
  @objc func authenticate(_ call: CAPPluginCall) {
    ...
    if let handlerClassName = getString(call, PARAM_CUSTOM_HANDLER_CLASS) {
      if let handlerInstance = self.getOrLoadHandlerInstance(className: handlerClassName) {
        ...
        handlerInstance.getAccessToken(
          viewController: (bridge?.viewController)!, call: call,
            ...
            if resourceUrl != nil {
              ...
              client.get(resourceUrl!) { result in
                ...
                case .success(let response):
                  if var jsonObj = try? JSONSerialization.jsonObject(
                    with: response.data, options: []) as? JSObject
                  {
      ...
      guard let baseUrl = getOverwritableString(call, PARAM_AUTHORIZATION_BASE_URL),
        !baseUrl.isEmpty
      else {
      ...
      if baseUrl.contains("appleid.apple.com"), #available(iOS 13.0, *) {
        ...
        guard let responseType = getOverwritableString(call, PARAM_RESPONSE_TYPE),
        ...
        guard let redirectUrl = getOverwritableString(call, PARAM_REDIRECT_URL),
          !redirectUrl.isEmpty
        else {
        ...
        var oauthSwift: OAuth2Swift
        if let accessTokenEndpoint = getOverwritableString(call, PARAM_ACCESS_TOKEN_ENDPOINT),
          !accessTokenEndpoint.isEmpty
        {
        ...
        let callParameter: [String: Any] =
          getOverwritable(call, PARAM_ADDITIONAL_PARAMETERS) as? [String: Any] ?? [:]
        let additionalParameters = buildStringDict(callParameter)

        let requestState =
          getOverwritableString(call, PARAM_STATE) ?? generateRandom(withLength: 20)
  ...
  @objc func logout(_ call: CAPPluginCall) {
    if let handlerClassName = getString(call, PARAM_CUSTOM_HANDLER_CLASS) {
      if let handlerInstance = self.getOrLoadHandlerInstance(className: handlerClassName) {
        let success: Bool! = handlerInstance.logout(
          viewController: (bridge?.viewController!)!, call: call)
        if success {
          call.resolve()
        } else {
  ...
  private func handleAuthorizationResult(
    ...
    case .success(let (credential, response, parameters)):
      ...
      if resourceUrl != nil && !resourceUrl!.isEmpty {
        ...
        let callParameter: [String: Any] =
          getOverwritable(call, PARAM_ADDITIONAL_RESOURCE_HEADERS) as? [String: Any] ?? [:]
        let additionalHeadersDict = buildStringDict(callParameter)

        self.oauthSwift!.client.get(
          resourceUrl!,
          headers: additionalHeadersDict
        ) { result in
          switch result {
          case .success(let resourceResponse):
            do {
              ...
              var jsonObj =
                try JSONSerialization.jsonObject(with: resourceResponse.data, options: [])
                as! JSObject
              // send the access token to the caller so e.g. it can be stored on a backend
              // #154
              if let accessTokenResponse = response {
                let accessTokenJsObject =
                  try? JSONSerialization.jsonObject(with: accessTokenResponse.data, options: [])
                  as? JSObject
                jsonObj.updateValue(
                  accessTokenJsObject!, forKey: self.JSON_KEY_ACCESS_TOKEN_RESPONSE)
              }
        ...
        do {
          var jsonObj = JSObject()
          let accessTokenJsObject =
            try? JSONSerialization.jsonObject(with: responseData, options: []) as? JSObject
  ...
  private func getConfigObjectDeepest(_ options: [AnyHashable: Any?]!, key: String) -> [AnyHashable:
    Any?]?
  {
  ...
  private func getOrLoadHandlerInstance(className: String) -> OAuth2CustomHandler? {
    guard
      let instance = self.getHandlerInstance(className: className)
        ?? self.loadHandlerInstance(className: className)
    else {
      return nil
    }
    return instance
...
extension GenericOAuth2Plugin: ASWebAuthenticationPresentationContextProviding {
...
extension GenericOAuth2Plugin: ASAuthorizationControllerDelegate {
  ...
  func handleSignInWithApple(_ call: CAPPluginCall) {
    ...
    if getValue(call, PARAM_IOS_USE_SCOPE) as? Bool != nil {
